...
class HorseshoeLogisticRegHierarchical(Distribution):
    def __init__(self, X, y) -> None:
        self.X = X
        self.y = y
    ...
    def logprob(self, beta, lamda, tau): #non-centered
        #priors
        lprob = jnp.sum(
            norm.logpdf(beta, loc=0., scale=jnp.exp(lamda+tau)) +
            gamma.logpdf(jnp.exp(lamda), a=.5, loc=0., scale=2.) + lamda
        ) + gamma.logpdf(jnp.exp(tau), a=.5, loc=0., scale=2.) + tau
        #likelihood
        logit = jnp.sum(self.X * beta, axis=1)
        p = jnp.clip(expit(logit), a_min=1e-6, a_max=1-1e-6)
        # p = jsp.special.expit(logit)
        lprob += jnp.sum(bernoulli.logpmf(self.y, p))
        return lprob
...
class MultivariateT:
    ...
    def __init__(self, alpha=1, d=100) -> None:
        self.alpha = alpha
        self.d = d
        self.y = 0

    def model(self, y=None):
        nu = numpyro.sample('nu', dist.Gamma(self.alpha, self.alpha))
        a = numpyro.sample('a', dist.Normal(0, 1).expand([self.d]))
        sigma = numpyro.sample('sigma', dist.HalfCauchy(1.))
        C = jnp.outer(a,a) + jnp.eye(self.d)*(sigma**2 + 1e-3)
        L = jnp.linalg.cholesky(C)
        numpyro.sample('z', dist.MultivariateStudentT(df = nu, 
                                                   loc = jnp.zeros(self.d), 
                                                   scale_tril = L)
                          )
    def initialize_model(self, rng_key, n_chain):
